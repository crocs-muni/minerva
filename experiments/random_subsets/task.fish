...
function run_atk
    ...
    for i in (seq 5)
        set n_seed (random 0 1099511627776)
        set params (echo "$argv[4]" | jq ".attack.seed_n = $n_seed")
        echo $params >$params_temp
        set successful 0
        for j in (seq 100)
            set start (date +%s)
            ...
            set d_seed (echo $out | grep -o "Random seed: [0-9]*" | grep -o "[0-9]*")
            ...
            echo "$n_seed,$d_seed,$found,$duration,$block_size,$info,$liars,$real_info,$bad_info,$good_info,$liar_positions,$result_normdist,$result_row"
            if [ \( $successful -ge 1 \) -a \( $j -ge 10 \) ]
                break
            end
        end
    end
