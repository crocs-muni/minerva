...
AttackRun = namedtuple("AttackRun", ("dataset", "bounds", "method", "recenter", "c", "N", "d","seed", "success", "time", "block_size", "info", "liars", "real_info", "bad_info", "good_info", "liar_positions", "normdist", "row"), defaults={"liar_positions": None, "normdist":None, "row": None})
                ...
                with open(fname, newline="") as f:
                    ...
                    for line in r:
                        try:
                            ...
                            if len(line) >= 12:
                                row = int(line[11])
                            run = AttackRun(data_type, bound_type, method_type, recenter, c, n, d, seed, success, time, block_size, info, liars, real_info, bad_info, good_info, liar_positions, normdist, row)
                            runs.add(run)
                        except:
                            continue
    return runs
