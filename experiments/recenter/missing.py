...
if __name__ == "__main__":
    ...
    for run in runs:
        if run.dataset in data_types and run.bounds in bound_types and run.method in method_types and run.recenter in recenter_types:
            present.add((run.N, run.d, run.dataset, run.bounds, run.method, run.recenter))
    for n in n_list:
        for d in d_list:
                ...
                for bounds in bound_types:
                    for method in method_types:
                        for recenter in recenter_types:
                            if (n, d, dataset, bounds, method, recenter) not in present:
                                print(dataset, bounds, method, recenter, n, d, sep="_")
