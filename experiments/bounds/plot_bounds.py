...
if __name__ == "__main__":
    ...
    for data_type in sorted(data_types):
        ...
        ax.step(range(args.d), bounds, label=data_type + f" ({sum(bounds)}b)", where="post")
    ...
    for data_type in sorted(data_types):
        for bound_type in sorted(bound_types):
            ...
            ax.plot(range(args.d), mean_residuals, label="_".join((data_type, bound_type)) + f" residuals ({np.mean(mean_residuals):.2f})")
            
            ax.step(range(args.d), bounds, label="_".join((data_type, bound_type)) + f" ({sum(bounds)}b) ({mean_err:.2f}e, {std_err:.2f})", where="post")
            if max(bounds) > max_bound:
                max_bound = max(bounds)
