...
def load_data(data_type, bound_type, method_type, recenter, e, d_range, n_range):
    runs = set()
    fnames = glob(join("run", "{}_{}_{}_{}_{}*.csv".format(data_type, bound_type, method_type, recenter, e)))
    if len(fnames) == 0:
        return runs
    for d in d_range:
        for n in n_range:
            fname = join("run","{}_{}_{}_{}_{}_{}_{}.csv".format(data_type, bound_type, method_type, recenter, e, n, d))
            if fname in fnames:
            ...
            for method in ("svp", "np", "round", "sieve"):
                for recenter in ("yes", "no"):
                    for e in ("1", "2", "3"):
                        ...
                        if loaded:
                            print(data, bounds, method, recenter, e, len(loaded))
                            print(len(all_runs))
