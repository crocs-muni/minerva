...
if __name__ == "__main__":
    ...
    for run in runs:
        if run.dataset in data_types and run.bounds in bound_types and run.method in method_types and run.recenter in recenter_types and run.e in e_types:
            ...
            if len(e_types) != 1:
                name_parts.append(str(run.e))
            data = datas.setdefault((run.dataset, run.bounds, run.method, run.recenter, run.e)
, {"name": " ".join(name_parts), "runs": set()})
    ...
    if figure == "success":
        ...
        plot_dim(datas, fig, map2liarinfo, int(dim_match.group(1)), "Average error info (bits)")
